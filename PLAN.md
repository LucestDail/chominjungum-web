...
### Phase 1 — 학습지 (서버 불요)
- [ ] 문항 입력·분해·글리프 렌더(editor/compact 프로필 준용)
- [ ] 자모 가리기 4모드 + 개별/전체 토글
- [ ] 인쇄 레이아웃(@page), 16자·20줄 제약
- **완료 기준**: jammin `/test`와 같은 학습지를 인쇄 미리보기로 동등 재현

### Phase 2 — 서버 + 웹 응시
- [ ] Spring Boot + PostgreSQL + Flyway, 교사 인증(JWT)
- [ ] 학급·학생·문항·과제 CRUD
- [ ] 참여코드 입장 → 웹 응시 → 서버 재채점 → 즉시 피드백
- **완료 기준**: 교사가 과제를 열고 다른 브라우저에서 학생이 응시·제출, 점수가 DB에 남음

### Phase 3 — 업싱크 (앱 연계)
- [ ] `sync_protocol` v2(matches·sessionId, optional)
- [ ] Flutter 교사앱: 로컬 큐 + 업로드 버튼 + 재시도
- [ ] `POST /api/sync/sessions` 멱등 수신, 기기↔명단 매핑 UI
- **완료 기준**: 인터넷을 끊고 교실 모드로 수업 → 연결 후 업로드 → 서버 성적부에 반영. 같은 배치 2회 전송해도 중복 없음

### Phase 4 — 성적·리포트
- [ ] 학생·학급 대시보드, 취약 자모 분석
- [ ] 성적표 PDF·CSV
- **완료 기준**: 한 학기치 더미 데이터로 리포트 생성
