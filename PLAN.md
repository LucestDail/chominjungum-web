...
| R2 | **폰트/SVG 라이선스** | KCC 도담도담체 재배포 조건, SVG 자산 출처 | ✅**해소**(§10) — OFL 확인·번들, `frontend/public/fonts/KCCDodamdodam-LICENSE.txt` 고지. 자산 출처는 `frontend/test/hangul-assets-provenance.test.ts` 가 강제 |
...
- ✅ **인쇄 확인 완료**(2026-09-04, §13.7) — `frontend/verify/` 로 헤드리스 스크린샷 +
  인쇄 PDF 를 뽑아 jammin 원본 렌더러 복사본과 **겹쳐 대조**했다(초·중성 완전 일치,
  종성만 1px 차이이고 우리가 정확). 사람 눈 확인이 필요한 항목이 아니게 됐다.
...
- **취약 자모가 학습지로 이어진다**: 분석 결과의 자모 코드가 가리기 기능과 같은 체계라,
  ...
  ✅**연결 UI 도 구현·배선 완료**(`features/worksheet/handoff.ts` ←
  `ClassroomReportPanel.vue` → `WorksheetEditor.vue`). "후속"이라 적힌 채 남아 있었다(09-10 정정).
...
| chominjungum-web (지금) | **학습지=도담도담체+골든 옐로우 / 콘솔=단색 실무 톤**(2026-09-03 이원화 완료) |

- ✅ **폰트 결정·번들 완료**(2026-09-03) — KCC 도담도담체는 **OFL**(한국저작권위원회 공유마당)임을 확인해
  `frontend/public/fonts/` 에 번들하고 고지를 동봉했다(`KCCDodamdodam-LICENSE.txt`).
  앱(`chominjungum`)에도 같은 폰트가 들어가 세 곳의 결과물이 같은 얼굴을 갖는다.
...
### 13.3 방향성 — ✅ **2026-09-16 결정됨**

| 갈림길 | 결정 | 근거 |
|---|---|---|
| 교사 개인 도구 vs 학교 단위 서비스 | ✅ **교사 개인 도구** | 09-04 상품화 평가가 *"평가 5개 중 유일하게 매출 경로가 있다"* 고 본 근거가 정확히 이것이다 — **교육청 개인정보 심사를 구조적으로 회피**하고 **B2G 가 아닌 교사 대상 B2C**. 학교 단위로 가면 학생 개인정보를 모으게 되어 그 강점을 스스로 내놓는다 |
| 학부모 리포트 열람 | ❌ **안 한다** | 학부모가 보려면 **학생↔학부모를 잇는 계정 체계**가 필요하고 그것이 곧 개인정보 수집이다. 위와 같은 이유. 필요하면 **교사가 PDF 를 출력해 전달**한다(앱에 PDF 출력은 이미 있다) |

⇒ **인증·개인정보 설계는 지금 형태를 유지한다.** 이 결정이 바뀌면 `docs/sync-protocol.md` 와 업싱크 소유자 모델부터 다시 봐야 한다.
- ✅ **상품화 평가 완료**(2026-09-04, `ASSESSMENT-2026-09.md` §3 + 기술 실사 `ASSESSMENT-chominjungum-web-2026-09.md`).
  판정 = **🥇 Tier 1 — 평가 대상 5개 중 유일하게 매출 경로가 있다**(무중앙서버로 교육청 개인정보 심사를
  구조적으로 우회, B2G 가 아니라 **교사 대상 B2C**). 두 문서 모두 워크스페이스 루트(git 아님·IP 라 커밋 금지)

### 13.4 남은 검증

- [x] 학습지 인쇄 미리보기 — **2026-09-04 확인**(§13.7). A4 1쪽, 칸 정렬 정확, 도담도담체 렌더 정상
- [x] 글자 크기별 자모 위치 · 받침 · 특수문자 — 원본과 겹쳐 대조 완료
- [x] 교사 콘솔 · 학생 응시 · 채점 결과 — 실데이터로 확인
- [ ] 교실 2대(교사앱 + 학생앱) 왕복 리허설 → 업싱크까지 **(실기기 필요, 유일한 잔여)**

### 13.5 진행 (2026-09-03)

✅ **디자인 이원화** — 학습지(도담도담체+옐로우) / 콘솔(단색 실무 톤)을 CSS 변수로 분리. 폰트는 OFL 확인 후 번들(고지 동봉).
✅ **세부 컴포넌트 4종** — 학습지(드래그 정렬·가리기 프리셋·인쇄 미리보기) / 응시(한 문항씩·진행 표시·큰 글씨·정답 TTS) / 리포트(점수 추이 차트·취약 자모→학습지 연결) / 콘솔(문항 검색·학년·단원 태그·명단 일괄 입력).
✅ **자모 배치 재작성** — 자산 실측 비율로 마법 숫자 제거(§13.6).

> ✅ **2026-09-04 에 "눈으로만 확인 가능한 것들"을 자동화했다**(§13.7, `frontend/verify/`).
> jammin 원본 렌더러의 **바이트 복사본과 겹쳐 대조**하는 방식이라 jammin 서버가 필요 없다.

- [x] 글자 크기 60%↔120% 에서 초/중/종 위치 — 원본과 겹쳐 대조 완료
- [x] 받침 있는 글자(값·없다), 쉼표·마침표 위치 — 완료(종성만 1px 차이, **우리가 정확**)
- [x] 인쇄 미리보기가 jammin 원본과 같아 보이는지 — 완료
- [x] 도담도담체 렌더 — 완료
- [ ] 교실 2대 왕복 리허설 **(실기기 필요 — 앱 저장소의 `docs/REHEARSAL.md` 참고)**

### 13.6 자모 배치의 근거 (2026-09-03)

SVG 83개를 전수 조사하니 부위별 캔버스가 정확히 하나씩이고 유니코드 그룹과 1:1이었다.
중성(400.9)+종성(221.1)=622.0 ≈ 정사각 상자(623.6) — **조각들은 한 글자 상자에서 부위별로 크롭된 것**이다.
jammin `profiles.js` 의 픽셀 수치를 이 비율로 역산하면 전부 "상자 ≈93px"로 수렴한다.
즉 원본 마법 숫자는 같은 비율을 손으로 계산해 반올림한 값이었다.

⇒ `hangul-metrics.ts` 가 비율 정본이고 `HangulCell` 은 `size` prop 하나로 스케일한다.
자산 viewBox 를 실파일에서 읽어 상수와 대조하는 테스트가 있어, SVG 를 다시 그리면 즉시 깨진다.


### 13.7 시각 검증 (2026-09-04)

"눈으로 봐야 아는 것"을 **헤드리스로 자동화**했다. `frontend/verify/` 참고.

**방법**: jammin 의 글리프 렌더링은 순수 클라이언트 JS(`glyph-renderer.js`)라
**jammin 서버 없이** 원본과 똑같은 DOM 을 만들 수 있다. 원본 파일을 바이트 그대로
복사해 두고(`verify/jammin-ref/`, 해시 대조) 같은 화면에서 새 구현과 겹쳐 본다.
자산도 동일하므로 색을 입혀 겹치면 어긋남이 즉시 드러난다.
스크린샷·인쇄 PDF 는 시스템 Chrome 으로 뽑는다(`npm run verify:shot`, 의존성 없음).
...
|---|---|
| 초성·중성 | 완전히 겹침 |
| 종성 | 1px 차이. 원본이 `top:59` 로 반올림한 자리를 자산 실측값 59.79 로 쓴 것 — **새 구현이 정확** |
| 크기 48~180px | 상대 위치 유지 |
| 가리기 4모드 | 정상 |
| 인쇄 | A4 1쪽, 정렬 정확, 도담도담체 렌더 |

**고친 것 3건**

1. `HangulCell` 의 조각이 전역 `img { max-width: 100% }` 를 받고 있었다. 지금은 모든
   조각이 폭 100% 이하라 증상이 없지만, 상자보다 넓게 놓아야 하는 글리프를 그 규칙이
   잘라낸다 → `max-width: none`. (같은 규칙이 원본 마크업에 걸리면 특수문자가 폭 0 이
   되어 사라진다. 대조 화면이 그것부터 잡아냈다.)
2. 인쇄물에 편집용 "N칸" 배지가 찍혔다 → 인쇄에서 숨김.
3. 점수 추이 차트에서 y축 눈금 "100" 과 첫 회차 라벨이 6px 간격으로 붙어 겹쳐 읽혔다
   → 양 끝 라벨을 플롯 안쪽으로(start/end), 위쪽 끝에 붙은 점은 라벨을 아래로.

**원본(jammin)에서 확인한 결함 3건 — 따라가지 않는다**

문장 단위로 나란히 그려 보니 원본에는 이런 문제가 있다. 준용 대상은 받아쓰기 **로직**이고
아래는 레이아웃 결함이므로 새 구현의 현재 방식(폭을 상자에 맞추는 한 가지 규칙)을 유지한다.

- 숫자가 칸을 크게 넘친다 — `specialHeights.default: 136` 은 쉼표용 값인데 정사각인
  숫자 SVG 에도 적용된다.
- 특수문자 칸이 옆 칸을 밀어 한 줄의 세로 정렬이 어긋난다.
- 원고지 배경이 글자보다 13px 오른쪽으로 밀려 있다(`background.left: 13` vs 글자 `left: 0`).

**검증 화면 쓰는 법**

```bash
npm run dev                       # http://localhost:5180/verify.html
npm run verify:shot               # png + 인쇄 pdf → frontend/verify/out/
```

`?only=B&zoom=2&diagnose` 로 한 섹션만 크게, 조각 외곽선까지 볼 수 있다.
서버가 필요한 화면은 계정을 URL 로 준다:
`?only=H&student&email=&pw=` (교사 콘솔+학생 상세) ·
`?only=H&join=<코드>&name=<이름>&play=<쓸 답>` (학생 응시→제출→채점 결과).
